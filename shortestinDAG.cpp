...
struct lmincomp{//if evaluated at critical_value the value is leq 0 then the minimum is earlier else its later
  ...
  lmincomp(linexp e1,linexp e2){
    if(e1.a==e2.a){
      ...
      if(e1.b<e2.b) earlier=later=e1;
      else earlier=later=e2;
      critical_value=0;
    }
    else{
...
double shortestpath(vector<vector<edge>> &adj){ //calculate length of shortest path in topologically sorted DAG given by adjacency list from first vertex to last
  ...
  for(int v=0;v<adj.size();v++){
    for(auto e:adj[v]){
      ...
      else dist[e.dest]=min(dist[e.dest],dist[v]+e.cost);
    }
  }
  return dist[adj.size()-1];
}

vector<vector<ledge>> readgraph(string s){
  stringstream ss(s);
  int vert, edge;
  ss>>vert>>edge;
...
double bruteforce_fracpaths(double& best,const vector<vector<ledge>>& adj,int v=0,double f=0, double g=0){
  if(v==adj.size()-1){
    best=min(best,f/g);
    return best;
  }
...
void print_bruteforce_fracpaths(vector<int>& path,const vector<vector<ledge>>& adj,int v=0,double f=0, double g=0){
  if(v==adj.size()-1){
    cout<<"path: \n\t\t\t";
...
int main(){
  ...
  for(int testcase=0;testcase<100;testcase++){
    ...
    //cout<<"shortest path length is "<<sol<<endl;
